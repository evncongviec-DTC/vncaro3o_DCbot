...
class BotThread(QThread):
    ...
    side_detected = pyqtSignal(bool) 
    history_signal = pyqtSignal(list, int) 
    ...
    def save_match_log(self, board, result, opp_elo=""):
        ...
        data_merged_fn = f"logs/vncaro_human_matches_{day_str}.dat"
        try:
            with open(data_merged_fn, 'a', encoding='utf-8') as f:
                ...
                f.write(")\n")
            self.log(f"💾 Đã nối thêm 1 trận vào Data tổng: {data_merged_fn}")
        except Exception as e:
            self.log(f"❌ Lỗi Data: {e}")
