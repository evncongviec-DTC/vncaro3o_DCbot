# ==============================================================
# GAME_RULES.PY - Module luật chơi Caro (3 bộ luật tích chọn)
# ==============================================================
# LUẬT 1 (Gomoku):    5 quân liền = thắng (dù bị chặn 2 đầu)
# LUẬT 2 (Caro):      5 quân chặn 2 đầu = KHÔNG thắng, 6+ quân = thắng
# LUẬT 3 (Custom):    3 ô trung lập tam giác + Gomoku (5 quân = thắng)
# ==============================================================
import numpy as np
import random
import math
...
class CaroGame:
    def __init__(self, rule_type=3, *args, **kwargs):
        ...
        self.neutral_cells = []
        self.move_history = [] # list of (r, c)
        
        # === CẤU HÌNH ĐỘ KHÓ AI (LEVELS) ===
        # Dùng cho các bản nâng cấp sau (AI Level 1 -> 10)
        self.attack_weight = 1.2   # Trọng số tấn công (Càng cao càng thích tạo bẫy, gài thế)
        self.defense_weight = 1.1  # Trọng số phòng thủ (Càng cao càng thích đi lấp lỗ hổng của địch)
    ...
    def generate_neutral_cells(self):
        ...
        for r, c in pts:
            self.board[r, c] = 2 # 2 là ô trung lập
    ...
    def get_valid_moves(self, current_player_is_X):
        ...
        # Đếm số quân cờ thật (X=1, O=-1), KHÔNG đếm ô trung lập (2)
        total_pieces = int(np.sum((self.board == 1) | (self.board == -1)))
        
        # Nếu là nước X đầu tiên (chưa có quân nào trên bàn)
        if total_pieces == 0:
            for r in range(BOARD_SIZE):
                for c in range(BOARD_SIZE):
                    if self.board[r, c] == 0:
                        # Phải cạnh ô trung lập và hướng ra khoảng trống >= 5 ô
                        is_adj = False
                            ...
                            for nc in range(BOARD_SIZE):
                                if self.board[nr, nc] == 2:
                                    ...
                                    if max(abs(dr), abs(dc)) == 1:
                                        is_adj = True
                                        # Đếm số ô trống theo hướng từ N -> X
                                        count = 0
                                        cr, cc = r + dr, c + dc
        ...
        # Nếu là nước X thứ 2 (có 2 quân: X1 và O1) -> Áp dụng Open 4
                ...
                for r in range(BOARD_SIZE):
                    for c in range(BOARD_SIZE):
                        if self.board[r, c] == 0:
                            ...
                            # Luật chuẩn: Cách X1 ĐÚNG 3 ô TRỐNG (tức là khoảng cách toán học dist == 4)
                            if dist == 4 and 3 <= r < BOARD_SIZE - 3 and 3 <= c < BOARD_SIZE - 3:
                                valid_moves[r * BOARD_SIZE + c] = 1
                if np.sum(valid_moves) > 0:
                    return valid_moves

        # Các nước bình thường (đánh vào ô trống)
        current_player = 1 if current_player_is_X else -1
        
        # === KHOANH VÙNG CHIẾN SỰ (Bán kính 4 ô - Tối đa 3 ô trống) ===
        ...
        # === BẢN NĂNG SINH TỒN: Tìm nước THẮNG NGAY hoặc CHẶN THẮNG ===
        # Đưa lên đầu tiên: bỏ qua mọi luật rìa nếu là nước quyết định sinh tử!
        ...
        for r in range(BOARD_SIZE):
            for c in range(BOARD_SIZE):
                if self.board[r, c] == 0 and proximity_mask[r, c]:
                    ...
                    self.board[r, c] = 0
        if has_block:
            # Chấm điểm phụ để chọn nước chặn tốt
        ...
        # === Không có nước thắng/chặn tức thì → CHẤM ĐIỂM CHIẾN THUẬT ===
        ...
        if np.sum(scores) > 0:
            ...
            # === HỆ THỐNG ÉP BUỘC PHẢN ỨNG ===
            # Lọc theo mức đe dọa nhưng GIỮ NGUYÊN ĐIỂM SỐ
            # để MCTS phân biệt nước chặn nào vừa chặn vừa có lợi cho mình
            if max_score >= 100000:
            ...
            # Lưu cache điểm cho MCTS dùng (thay vì tính lại)
            self._cached_scores = scores.copy()
            
            # Trả valid_moves (binary) cho compatibility
    ...
    def get_heuristic_policy(self, current_player_is_X):
        ...
        total = np.sum(scores)
        if total > 0:
            # Lấy mũ để khuếch đại sự khác biệt giữa các điểm đánh giá
    ...
    def evaluate_cell(self, r, c, player):
        ...
        # Đếm đe dọa để phát hiện Bẫy (Fork)
        four_count = 0
        open3_count = 0
        ...
        for dr, dc in dirs:
            ...
            if pattern_type in ('C4', 'B4'):
                ...
                four_count += 2  # Open 4 tính như 2 đường C4 vì nó không thể bị chặn 1 phát
        
        # Phát hiện Bẫy liên hoàn (Fork/Double Threat)
        if four_count >= 2:
            total_score += 60000  # Bẫy 4x4
        elif four_count >= 1 and open3_count >= 1:
            total_score += 40000  # Bẫy 4x3
        elif open3_count >= 2:
            total_score += 30000  # Bẫy 3x3
    ...
    def score_direction(self, r, c, dr, dc, player):
        ...
        # Đếm quân liên tiếp theo chiều dương
        count_pos = 0
        gap_pos = 0
        after_gap_pos = 0
        nr, nc = r+dr, c+dc
        while 0<=nr<BOARD_SIZE and 0<=nc<BOARD_SIZE and self.board[nr,nc] == player:
            count_pos += 1
            nr, nc = nr+dr, nc+dc
        # Kiểm tra gap (ô trống) rồi tiếp tục đếm quân (cho Broken pattern)
        ...
        # Đếm quân liên tiếp theo chiều âm
        count_neg = 0
        gap_neg = 0
        after_gap_neg = 0
        nr, nc = r-dr, c-dc
        ...
        consecutive = 1 + count_pos + count_neg  # Quân liên tiếp qua ô (r,c)
        total_with_gap = consecutive + (after_gap_pos if gap_pos else 0) + (after_gap_neg if gap_neg else 0)
        
        # === KIỂM TRA KHÔNG GIAN SỐNG (ROOM) ===
        # Đếm tổng số ô "thở được" trên hướng này (quân mình + ô trống liên tục)
        # Nếu < 5 → đường này BẾ TẮC, không bao giờ thành 5 được → bỏ qua
        room = consecutive
        # Đếm ô trống phía dương (cho đến khi gặp quân địch hoặc mép)
        ...
        # Hệ số nhân không gian sống (Room Multiplier)
        # Mỗi ô không gian sống sẽ cộng thêm 1% điểm (room 5 = x1.05, room 9 = x1.09)
        # Giúp AI đánh giá chính xác khoảng trống thực tế (có tính cả tường và ô trung lập)
    ...
    def get_state_tensor(self, current_player):
        # Tạo input 3 channel: [Quân mình, Quân địch, Ô trung lập]
